...
# ----------------------------------------------------
# 5. STORAGE (Cloudflare R2 - DocumentaciÃ³n de IntenciÃ³n)
# ----------------------------------------------------
# NOTA: En un flujo real usarÃ­amos el provider `cloudflare` para
# crear el bucket R2. Por propÃ³sitos de la demostraciÃ³n SDLC,
# se representa AWS aquÃ­ y R2 configurado desde el portal para evitar pagos de Egress.
...
resource "aws_security_group" "ecs_sg" {
  ...
  ingress {
    ...
    to_port     = 8000
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # TÃ­picamente conectado a un ALB (Load Balancer)
  }
  egress {
    from_port   = 0
    to_port     = 0
